function [sob_vsla, sof_vsla, kg_dif] = A_BRFv2_single_dif(tto, CIo, CIy1, CIy2, LAI, lidf, hc0)
% A_BRFV2_SINGLE_DIF Calculates the hemispherical integration of single scattering and gap probabilities.
% 
% DESCRIPTION:
% This function performs a double numerical integration over the upper hemisphere 
% (zenith angle: 0 to pi/2, azimuth angle: 0 to 2*pi) using an 8-point Gaussian quadrature method.
% It calculates the diffuse transmittance and integrated single scattering contributions.
%
% INPUTS:
%   tto  - Solar/illumination zenith angle [degrees]
%   CIo  - Clumping index at the illumination direction
...
%   kg_dif   - Integrated gap probability (diffuse transmittance)
...
% over thetaL (View Zenith Angle integration limits: 0 to pi/2)
...
% Define limits of integration and the conversion factors for integration
% over phiL (View Azimuth Angle integration limits: 0 to 2*pi)
...
for i = 1:8
    ...
    for j = 1:8
        ...
        tta  = neword_tL * 180 / pi;   % Convert observer zenith angle to degrees
