function [kc, kg] = sunshade(tts, tto, psi, Gs, Go, CIs, CIo, LAI, hc0)
% SUNSHADE Calculates the probabilities of viewing sunlit foliage and sunlit soil.
%
% DESCRIPTION:
% This function computes the bi-directional gap probabilities considering the 
% hotspot effect based on the Kuusk (1995) exponential correlation formulation. 
% The canopy is vertically discretized into elementary layers to numerically 
% integrate the joint probability of a gap in both the solar and viewing directions.
%
% INPUTS:
...
%   hc0 - Canopy structural parameter / Hotspot parameter (H)
%
% OUTPUTS:
%   kc  - Probability of viewing sunlit foliage (sunlit crown fraction)
%   kg  - Probability of viewing sunlit soil (bi-directional gap fraction, Pso)
...
% Case 2: General bi-directional calculation using canopy stratification
    ...
    d = 0.2;  % Characteristic leaf dimension / related to leaf shape & LIDF
    ...
    % Independent probabilities of viewing a leaf in solar and observation directions
    Ps = exp(k * xl * CIs * LAI);                                              
    Po = exp(K * xl * CIo * LAI);                                              
    ...
    % Take care of rounding errors to ensure joint probability doesn't exceed independent probabilities
...
function pso = Psofunction(K, k, CIs, CIo, LAI, q, dso, xl)
    if dso ~= 0
        alf = (dso / q) * 2 / (k + K);
        % Factor for correlation of Ps and Po with exponential decay
        pso = exp((K * CIo + k * CIs) * LAI * xl + sqrt(K * CIo * k * CIs) * LAI / alf * (1 - exp(xl * alf))); 
    else
        % Exact overlap (no separation)
