function [Qfdir_all, Qfyld_all, Qapar_all, Qpdir_bs] = get_rta_nosif_comment(nb, nf, ne, iwlfi, iwlfo, angle, Qins, Qind, rho_l, tau_l, rs, data_p_rho)
% GET_RTA_NOSIF_COMMENT Calculates optical radiative transfer without SIF emission.
%
% DESCRIPTION:
% This function simulates the optical radiative transfer within the canopy 
% taking into account multiple scattering. It solves the radiative transfer 
% equation by separating it into two classic sub-problems: 
%   1. The Black Soil (BS) problem (canopy scattering only).
%   2. The Soil (S) problem (soil-canopy multiple interactions).
% Note: Although variables might be named with an 'f' (like Qfdir), the 
% fluorescence excitation matrices (MfI, MbI) are set to zero in this routine 
% to isolate the pure optical/background scattering.
%
% INPUTS:
...
%
% OUTPUTS:
%   Qfdir_all  - Total directional exitance (Top of Canopy)
%   Qfyld_all  - Total yield within the canopy
%   Qapar_all  - Total Absorbed Photosynthetically Active Radiation (APAR)
%   Qpdir_bs   - Directional exitance from the BS problem only

% -------------------------------------------------------------------------
% 0. Initialize Fluorescence Matrices to Zero (No SIF emission)
MfI = zeros(nf, ne);
MbI = zeros(nf, ne);
Mf  = MfI + MbI;
...
% -------------------------------------------------------------------------
% Initialization: 11 represents successive scattering orders
...
for i = 1:11
    Qapar(:, i) = Qsig(:, i) .* (1 - wleaf);  % Absorbed photons
    MQ = Mf * Qsig(iwlfi, i);                 % Note: Mf is 0 here
...
Qind_s   = Qdown_bs .* rs;   % Soil illumination (includes 0-order transmittance)
...
for k = 1:8    % Loop for multiple bounces between soil and canopy
    ...
    if k == 1
        % First reflection from soil (direct + diffuse components)
    ...
    if (k == 1)
        % Add zero-order soil scattering modulated by GO 4-components
    ...
    % Prepare for next soil-canopy interaction cycle
    Qind_s = Qdown_ss .* rs;  
end
...
% -------------------------------------------------------------------------
Qfdir_all = Qfdir_bs + Qfdir_ss;      % Total directional exitance
Qfyld_all = Qfyld_bs + Qfyld_ss;      % Total canopy yield
Qapar_all = Qapar_bs + Qapar_ss;      % Total absorbed photons
Qpdir_bs  = Qfdir_bs;                 % Directional exitance from BS problem only
